...
const ErrorOfferMessage = {
  ...
  PRICE: `Сумма не может быть меньше 100`,
...
const schema = Joi.object({
  ...
  type: Joi.any().valid(OfferType.OFFER, OfferType.SALE).required().messages({
    'any.required': ErrorOfferMessage.TYPE
  }),
  price: Joi.number().integer().min(100).required().messages({
    'number.min': ErrorOfferMessage.PRICE
  }),
...
module.exports = (req, res, next) => {
  ...
  const {error: errors} = schema.validate(newOffer, {abortEarly: false});
  if (errors) {
    return res.status(HttpCode.BAD_REQUEST).send(errors.details.map((err) => err.message).join(`\n`));
  }
